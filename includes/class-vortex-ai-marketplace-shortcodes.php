...
class Vortex_AI_Marketplace_Shortcodes {
    ...
    public function register_shortcodes() {
        ...
        add_shortcode('vortex_registration', array($this, 'render_registration'));
        add_shortcode('vortex_artist_registration', array($this, 'render_artist_registration'));
        add_shortcode('vortex_collector_registration', array($this, 'render_collector_registration'));
        
        // Dashboard shortcodes
        add_shortcode('vortex_collector_dashboard', array($this, 'render_collector_dashboard'));
        add_shortcode('vortex_collector_subscription', array($this, 'render_collector_subscription'));
        add_shortcode('vortex_collector_purchases', array($this, 'render_collector_purchases'));
        add_shortcode('vortex_live_marketplace', array($this, 'render_live_marketplace'));
    ...
    /**
     * Render artist registration shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_artist_registration($atts) {
        ob_start();
        ...
        try {
            ...
            $atts = shortcode_atts(
                ...
                $atts, 
                'vortex_artist_registration'
            );
            
            // Sanitize parameters
            $atts['redirect'] = esc_url_raw($atts['redirect']);
            $atts['class'] = sanitize_html_class($atts['class']);
            $show_login = filter_var($atts['show_login'], FILTER_VALIDATE_BOOLEAN);
            
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/shortcodes/artist-registration.php');
            ...
            // Log the error
            $this->log_error('Artist Registration Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render user registration shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_registration($atts) {
        ...
        try {
            ...
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/shortcodes/user-registration.php');
            ...
            // Log the error
            $this->log_error('User Registration Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render collector registration shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_collector_registration($atts) {
        ob_start();
        ...
        try {
            ...
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/shortcodes/collector-registration.php');
            ...
            // Log the error
            $this->log_error('Collector Registration Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render collector dashboard shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_collector_dashboard($atts) {
        ...
        try {
            ...
            if (!is_user_logged_in()) {
                echo '<div class="vortex-login-required">';
                echo '<p>' . esc_html__('You must be logged in to view your collector dashboard.', 'vortex-ai-marketplace') . '</p>';
                echo '<a href="' . esc_url(wp_login_url(get_permalink())) . '" class="vortex-button">' . esc_html__('Log In', 'vortex-ai-marketplace') . '</a>';
                echo '</div>';
                return ob_get_clean();
            }
            
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/collector-dashboard.php');
            ...
            // Log the error
            $this->log_error('Collector Dashboard Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render collector subscription shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_collector_subscription($atts) {
        ob_start();
        ...
        try {
            ...
            $atts = shortcode_atts(
                ...
                $atts, 
                'vortex_collector_subscription'
            );
            
            // Check if user is logged in
            if (!is_user_logged_in()) {
                echo '<div class="vortex-login-required">';
                echo '<p>' . esc_html__('You must be logged in to access subscription plans.', 'vortex-ai-marketplace') . '</p>';
                echo '<a href="' . esc_url(wp_login_url(get_permalink())) . '" class="vortex-button">' . esc_html__('Log In', 'vortex-ai-marketplace') . '</a>';
                echo '</div>';
                return ob_get_clean();
            }
            ...
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/collector-subscription.php');
            ...
            // Log the error
            $this->log_error('Collector Subscription Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render live marketplace shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_live_marketplace($atts) {
        ob_start();
        ...
        try {
            ...
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/marketplace-live-artists.php');
            ...
            // Log the error
            $this->log_error('Live Marketplace Shortcode: ' . $e->getMessage());
        }
    ...
    /**
     * Render collector purchases shortcode
     * 
     * @param array $atts Shortcode attributes
     * @return string Rendered HTML
     */
    public function render_collector_purchases($atts) {
        ob_start();
        ...
        try {
            ...
            $atts = shortcode_atts(
                ...
                $atts, 
                'vortex_collector_purchases'
            );
            
            // Check if user is logged in
            if (!is_user_logged_in()) {
                echo '<div class="vortex-login-required">';
                echo '<p>' . esc_html__('You must be logged in to view your collection.', 'vortex-ai-marketplace') . '</p>';
                echo '<a href="' . esc_url(wp_login_url(get_permalink())) . '" class="vortex-button">' . esc_html__('Log In', 'vortex-ai-marketplace') . '</a>';
                echo '</div>';
                return ob_get_clean();
            }
            ...
            // Include template
            include(plugin_dir_path(dirname(__FILE__)) . 'public/partials/collector-purchases.php');
            ...
            // Log the error
            $this->log_error('Collector Purchases Shortcode: ' . $e->getMessage());
        }
