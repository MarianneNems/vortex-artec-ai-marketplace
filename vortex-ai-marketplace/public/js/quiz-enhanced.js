...
(function($) {
    ...
    class VortexCosmicQuiz {
        ...
        bindEvents() {
            ...
            $('.cosmic-question').on('mouseenter', function() {
        ...
        activateQuestionStar(questionKey) {
            const questionIndex = Array.from(this.questionBlocks).findIndex(block => 
                $(block).data('question') === questionKey
        ...
        generateCosmicPreview() {
            if (!this.zodiacSign || !vortexQuizData.zodiacSigns[this.zodiacSign]) return;
            ...
            Object.values(responses).forEach(response => {
                switch(response) {
                    case 'a': elementCounts.fire++; break;
                    case 'b': elementCounts.earth++; break;
                    case 'c': elementCounts.air++; break;
                    case 'd': elementCounts.water++; break;
                }
            });
        ...
        checkFormValidity() {
            ...
            const allConfirmed = this.confirmCheckboxes.length === this.confirmCheckboxes.filter(':checked').length;
            const allAnswered = this.questionBlocks.length === this.questionBlocks.filter('.answered').length;
        ...
        handleFormSubmit(e) {
            ...
            if (!this.validateForm()) {
                this.showCosmicMessage('Please complete all sections before launching your cosmic journey', 'error');
                return;
            }
            ...
            const data = {
                ...
                cosmic_analysis: {
                    ...
                    business_alignment: this.cosmicPreview.is(':visible') ? 
                        this.cosmicPreview.find('.preview-alignment h5').text() : null
                }
            };
        ...
        handleSubmitSuccess(response) {
            if (response.success) {
                ...
                this.showCosmicMessage('⚡ ' + (response.message || 'An error occurred while processing your cosmic journey.'), 'error');
            }
        }
        ...
        showCosmicMessage(message, type) {
            const messageClass = type === 'success' ? '.cosmic-success' : 
                                type === 'error' ? '.cosmic-error' : '.cosmic-info';
