...
(function ($) {
  ...
  const config = {
    ...
    endpoint: `${
      typeof VortexQuizConfig !== 'undefined' ? VortexQuizConfig.restUrl : '/wp-json/vortex/v1/'
    }optimized-quiz`,
    ...
    messages: {
      ...
      submissionSuccess:
        typeof VortexQuizConfig !== 'undefined'
          ? VortexQuizConfig.submissionSuccessMessage
          : 'Thank you! Horace is crafting your personalized 30-day milestone roadmap.',
      monthlyLimit:
        typeof VortexQuizConfig !== 'undefined'
          ? VortexQuizConfig.monthlyLimitMessage
          : 'You have already completed your assessment this month.',
    },
  };
  ...
  const QuizOptimizer = {
    ...
    addRealTimeValidation: function () {
      ...
      $('input[name="dob"]').on('change', function () {
        ...
        if ($(this).val() === '') {
          ...
          $(this).addClass('invalid');
          $(this).after(
            '<span class="validation-message error">Please enter a valid birth date</span>'
          );
        } else {
      ...
      $('input[name="pob"]').on('input', function () {
        ...
        if (value.length === 0) {
          ...
          $(this).addClass('invalid');
          $(this).after(
            '<span class="validation-message error">Please enter a valid location</span>'
          );
        } else {
          $(this).removeClass('invalid');
    ...
    addAutoSave: function () {
      ...
      $(document).on(
        'change input',
        `${config.formSelector} input, ${config.formSelector} textarea`,
        () => {
    ...
    restoreFormData: function (data) {
      ...
      if (data.dob) {
        $('input[name="dob"]').val(data.dob).trigger('change');
      }
      if (data.pob) {
        $('input[name="pob"]').val(data.pob).trigger('input');
      }
      if (data.tob) {
        $('input[name="tob"]').val(data.tob).trigger('change');
      }

      // Restore answers
      if (data.answers) {
        Object.keys(data.answers).forEach(key => {
          $(`input[name="${key}"][value="${data.answers[key]}"]`)
            .prop('checked', true)
            .trigger('change');
        });
      }
