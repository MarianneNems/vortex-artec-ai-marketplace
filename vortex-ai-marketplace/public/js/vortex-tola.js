...
(function ($) {
  ...
  window.VortexAPI = {
     ...
     */
    loadPlans: function () {
      this.apiRequest('plans', 'GET', null, (error, response) => {
        if (error) {
    ...
    selectPlan: function (e) {
      ...
      VortexAPI.apiRequest(`users/${userId}/plan`, 'POST', { plan: planId }, (error, response) => {
        if (error) {
    ...
    startRoleQuiz: function (e) {
      ...
      VortexAPI.showQuizModal(quizQuestions, answers => {
        const userId = vortexAjax.currentUserId;
        VortexAPI.apiRequest(
          `users/${userId}/role-quiz`,
          'POST',
          { answers: answers },
          (error, response) => {
            if (error) {
    ...
    pollGenerationStatus: function (jobId) {
      const pollInterval = setInterval(() => {
        VortexAPI.apiRequest(`api/generate/status/${jobId}`, 'GET', null, (error, response) => {
          if (error) {
    ...
    displayGenerationResults: function (generation) {
      ...
      generation.result_urls.forEach((url, index) => {
        html += `
    ...
    loadGenerationLimits: function () {
      ...
      VortexAPI.apiRequest(`users/${userId}/plan`, 'GET', null, (error, response) => {
        if (error || !response.plan_details) {
          return;
        }
    ...
    loadGallery: function () {
      const userId = vortexAjax.currentUserId;
      VortexAPI.apiRequest(`users/${userId}/collections`, 'GET', null, (error, response) => {
        if (error) {
    ...
    loadMilestones: function () {
      const userId = vortexAjax.currentUserId;
      VortexAPI.apiRequest(`users/${userId}/milestones`, 'GET', null, (error, response) => {
        if (error) {
          $('#milestonesTimeline').html(
            '<div class="vortex-error">Failed to load milestones</div>'
          );
          return;
        }
    ...
    mintNFT: function (imageUrl) {
      VortexAPI.apiRequest('nft/mint', 'POST', { image_url: imageUrl }, (error, response) => {
        if (error) {
