...
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-ai-agents.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-artist-categories.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-rankings.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-shuffle.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-thorius-api.php'; // THORIUS API endpoints
...
// Admin only includes
if (is_admin()) {
    require_once plugin_dir_path(__FILE__) . 'admin/cloe-shuffle-settings.php';
}
...
// Include artist education
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-artist-education.php';
...
function run_vortex_ai_marketplace() {
    ...
    // Initialize Rankings and Shuffle functionality
    $rankings = new Vortex_Rankings('vortex-ai-marketplace', VORTEX_VERSION);
    $shuffle = new Vortex_Shuffle('vortex-ai-marketplace', VORTEX_VERSION);
    
    $plugin = new Vortex_AI_Marketplace();
    $plugin->run();
    
    // Safe Thorius initialization
    if (class_exists('Vortex_Thorius')) {
        $thorius = Vortex_Thorius::get_instance();
        // Use indirect method calls if they exist
        if (method_exists($thorius, 'initialize')) {
            call_user_func(array($thorius, 'initialize'));
        }
        if (method_exists($thorius, 'init_widget')) {
            call_user_func(array($thorius, 'init_widget'));
        }
    }
}
...
// Enqueue shuffle and winners assets
function vortex_enqueue_shuffle_assets() {
    wp_enqueue_style(
        'vortex-shuffle',
        VORTEX_PLUGIN_URL . 'css/vortex-shuffle.css',
        array(),
        VORTEX_VERSION
    );
    ...
    wp_enqueue_style(
        'vortex-winners',
        VORTEX_PLUGIN_URL . 'css/vortex-winners.css',
        array(),
        VORTEX_VERSION
    );
    ...
    wp_enqueue_script(
        'vortex-shuffle',
        VORTEX_PLUGIN_URL . 'js/vortex-shuffle.js',
        array('jquery'),
        VORTEX_VERSION,
        true
    );
    
    wp_localize_script('vortex-shuffle', 'vortexShuffleData', array(
        'restUrl' => rest_url(),
        'restNonce' => wp_create_nonce('wp_rest'),
        'winnersUrl' => home_url('/winners'),
        'i18n' => array(
            'shuffleComplete' => __('Gallery shuffled successfully!', 'vortex-ai-marketplace'),
            'shuffleError' => __('Failed to shuffle gallery.', 'vortex-ai-marketplace'),
            'noWinners' => __('No winners found for this date.', 'vortex-ai-marketplace')
        )
    ));
}
add_action('wp_enqueue_scripts', 'vortex_enqueue_shuffle_assets');

// Register shuffle and winners shortcodes
function vortex_register_shuffle_shortcodes() {
    add_shortcode('vortex_shuffle_countdown', 'vortex_shuffle_countdown_shortcode');
    add_shortcode('vortex_daily_winners', array('Vortex_Shuffle', 'daily_winners_shortcode'));
}
add_action('init', 'vortex_register_shuffle_shortcodes');

// Shuffle countdown shortcode
function vortex_shuffle_countdown_shortcode($atts) {
    $atts = shortcode_atts(array(
        'title' => __('Next Gallery Shuffle', 'vortex-ai-marketplace'),
        'show_title' => true,
    ), $atts);
    
    $last_shuffle = get_option('vortex_last_gallery_shuffle', current_time('mysql'));
    $shuffle_interval = get_option('vortex_shuffle_settings', array('shuffle_interval' => 3));
    $interval_hours = isset($shuffle_interval['shuffle_interval']) ? intval($shuffle_interval['shuffle_interval']) : 3;
    
    ob_start();
    ?>
    <div class="vortex-shuffle-countdown" data-last-shuffle="<?php echo esc_attr($last_shuffle); ?>" data-interval-hours="<?php echo esc_attr($interval_hours); ?>">
        <?php if ($atts['show_title'] && !empty($atts['title'])) : ?>
            <div class="vortex-shuffle-countdown-label"><?php echo esc_html($atts['title']); ?></div>
        <?php endif; ?>
        <div class="vortex-shuffle-countdown-timer">00:00:00</div>
    </div>
    <?php
    return ob_get_clean();
}
...
function vortex_enqueue_registration_assets() {
    // Only enqueue on registration pages
    if (!is_page('register') && !is_page('artist-registration')) {
        return;
    }
    
    wp_enqueue_style(
        'vortex-registration',
        plugin_dir_url(__FILE__) . 'css/vortex-registration.css',
        array(),
        VORTEX_VERSION
    );
    ...
    // Artist quiz script
    if (is_page('artist-registration')) {
        wp_enqueue_script(
            'vortex-artist-quiz',
            plugin_dir_url(__FILE__) . 'js/vortex-artist-quiz.js',
            array('jquery'),
            VORTEX_VERSION,
            true
        );
        
        wp_localize_script('vortex-artist-quiz', 'vortex_quiz_data', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('vortex_quiz_nonce'),
            'feedback_title' => __('Business Strategist Feedback', 'vortex-ai-marketplace'),
            'tier_label' => __('Your Artist Tier', 'vortex-ai-marketplace')
        ));
    }
}
...
require_once VORTEX_PLUGIN_DIR . 'includes/shortcodes/class-vortex-ai-agents-shortcode.php';
require_once VORTEX_PLUGIN_DIR . 'includes/shortcodes/class-vortex-artist-categories-shortcode.php';
...
// Register AI agents shortcode
function vortex_register_ai_agents_shortcode() {
    add_shortcode('vortex_ai_agents', array('Vortex_AI_Agents', 'render_ai_agents_shortcode'));
}
add_action('init', 'vortex_register_ai_agents_shortcode');

// Enqueue AI agents scripts and styles
function vortex_enqueue_ai_agents_assets() {
    wp_enqueue_style(
        'vortex-ai-agents-css',
        plugin_dir_url(__FILE__) . 'css/vortex-ai-agents.css',
        array(),
        VORTEX_VERSION
    );
    ...
    wp_enqueue_script(
        'vortex-ai-agents-js',
        plugin_dir_url(__FILE__) . 'js/vortex-ai-agents.js',
        array('jquery'),
        VORTEX_VERSION,
        true
    );
...
add_action('wp_enqueue_scripts', 'vortex_enqueue_ai_agents_assets');

// Initialize AI Agents shortcode
function vortex_init_ai_agents_shortcode() {
    if (class_exists('Vortex_AI_Agents_Shortcode')) {
        Vortex_AI_Agents_Shortcode::init();
    }
}
add_action('init', 'vortex_init_ai_agents_shortcode'); 

// ... existing code ...
require VORTEX_PLUGIN_DIR . 'includes/class-vortex-artwork-manager.php';
require VORTEX_PLUGIN_DIR . 'includes/class-vortex-membership-manager.php';
require VORTEX_PLUGIN_DIR . 'includes/class-vortex-ai-agent-handler.php';
require VORTEX_PLUGIN_DIR . 'includes/class-vortex-seed-art-manager.php';
// ... existing code ...

// Include collector forum functionality
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-collector-forum.php';

// Register collector forum shortcodes
function vortex_register_collector_forum_shortcodes() {
    add_shortcode('vortex_forum', array('Vortex_Collector_Forum', 'render_forum_shortcode'));
    add_shortcode('vortex_create_post', array('Vortex_Collector_Forum', 'render_create_post_shortcode'));
}
add_action('init', 'vortex_register_collector_forum_shortcodes');

// Enqueue collector forum scripts and styles
function vortex_enqueue_collector_forum_assets() {
    wp_register_style(
        'vortex-collector-forum',
        plugin_dir_url(__FILE__) . 'css/vortex-collector-forum.css',
        array(),
        VORTEX_VERSION
    );
    ...
    wp_register_script(
        'vortex-collector-forum',
        plugin_dir_url(__FILE__) . 'js/vortex-collector-forum.js',
        array('jquery'),
        VORTEX_VERSION,
        true
    );
}
add_action('wp_enqueue_scripts', 'vortex_enqueue_collector_forum_assets');

// ... existing code ...

// Include real-time orchestration and collaboration components
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-websocket-server.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-realtime-orchestrator.php';
require_once plugin_dir_path(__FILE__) . 'includes/class-vortex-realtime-collaboration.php';

// Register collaboration assets
function vortex_register_realtime_collaboration_assets() {
    wp_register_style(
        'vortex-realtime-collaboration',
        plugin_dir_url(__FILE__) . 'css/vortex-realtime-collaboration.css',
        array(),
        VORTEX_VERSION
    );
    ...
    wp_register_script(
        'vortex-realtime-collaboration',
        plugin_dir_url(__FILE__) . 'js/vortex-realtime-collaboration.js',
        array('jquery'),
        VORTEX_VERSION,
        true
    );
}
add_action('init', 'vortex_register_realtime_collaboration_assets');

// Define plugin constants if not already defined
if (!defined('VORTEX_PLUGIN_FILE')) {
    define('VORTEX_PLUGIN_FILE', __FILE__);
}

// ... existing code ...

// Include AI agent real-time extensions
require_once plugin_dir_path(__FILE__) . 'class-vortex-cloe-rt-extension.php';
require_once plugin_dir_path(__FILE__) . 'class-vortex-huraii-rt-extension.php';
